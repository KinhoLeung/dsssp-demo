...
- UI：`DevicePanel`
  ...
  - 动作按钮：Ping（`{op:'ping', ts}`）、Send filters（`{op:'filters', payload:{filters, coefficients, sampleRate}}`）。
  - 日志面板：展示最近收发与错误。
...
## 扩展与对接建议
- 需要定协议时：在 `DevicePanel` 的 `sendJson` 参数处按实际帧格式组织；或在 `useDeviceLink` 新增编码/解码层，把 UI 事件映射为底层帧。
